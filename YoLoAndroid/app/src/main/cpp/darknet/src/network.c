...
void forward_network(network *netp)
{
    network net = *netp;
    int i;
    double dConvTimeUse = 0.;
    double dMaxPoolTimeUse = 0.;
    double dOtherTimeUse = 0.;
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
        if(l.delta){
        ...
        double dTimeUse = what_time_is_it_now();
        l.forward(l, net);
        dTimeUse = what_time_is_it_now() - dTimeUse;
        net.input = l.output;
        if(l.truth) {
            net.truth = l.output;
        }
        ...
        {
            LOGD("Layer %d: Conv ---- costs %f seconds.\n", i+1, dTimeUse);
            dConvTimeUse += dTimeUse;
        } else if(l.type == MAXPOOL)
        {
            LOGD("Layer %d: Max ---- costs %f seconds.\n", i+1, dTimeUse);
            dMaxPoolTimeUse += dTimeUse;
        } else
        {
            LOGD("Layer %d: Other ---- costs %f seconds.\n", i+1, dTimeUse);
            dOtherTimeUse += dTimeUse;
        }
    }
    LOGD("Convolutional Layers cost %f seconds.\n", dConvTimeUse);
    LOGD("MaxPool Layers cost %f seconds.\n", dMaxPoolTimeUse);
    LOGD("Other Layers cost %f seconds.\n", dOtherTimeUse);
